...
def test_simulate_image():
    ...
    image = simulate_single_image(image_parameters, 0) # added idx=0 to make tests work
    assert image.shape == (512, 512)


def test_execute_simulation():
    simulation = test_simulation_from_input_parameters()
    spsim.simulation_functions.simulation_as_dask_array(simulation) # added Simulation=simulation to make tests work
